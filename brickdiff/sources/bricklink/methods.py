...
class ItemMapping(Method):
    URL_GET_ELEMENT_ID = 'item_mapping/{type}/{no}'
    URL_GET_ITEM_NUMBER = '/item_mapping/{element_id}'

    def getElementID(self, item_no, color_id=None):
        params = {'color_id': color_id} if color_id is not None else {}
        return self.client.get(self.URL_GET_ELEMENT_ID.format(type='PART', no=item_no), params)

    def getItemNumber(self, element_id):
        return self.client.get(self.URL_GET_ITEM_NUMBER.format(element_id=element_id))
